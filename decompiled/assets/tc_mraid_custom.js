(function() {
    ...
    listeners['shakeDetection'] = [];
    listeners['longPressDetection'] = [];
    listeners['locationEvent'] = [];
    listeners['cameraEvent'] = [];
    listeners['rotationEvent'] = [];
    listeners['tiltEvent'] = [];
    ...
    mraid.util.audioVolumeChangeEvent = function(audioVolumeObject) {
      ...
      mraid.util.shakeDetectionEvent = function() {
          if (state === 'loading') return;
          mraid.util.fireEvent('shakeDetection');
      };

     mraid.util.longPressDetectionEvent = function() {
           if (state === 'loading') return;
           mraid.util.fireEvent('longPressDetection');
     };

    mraid.util.locationDetectionEvent = function(latitude, longitude) {
           if (state === 'loading') return;
           mraid.util.fireEvent('locationEvent', latitude, longitude);
    };

    mraid.util.cameraOpenEvent = function(image) {
           if (state === 'loading') return;
           mraid.util.fireEvent('cameraEvent', image);
    };

    mraid.util.rotationDetectionEvent = function(azimuth, pitch, roll) {
           if (state === 'loading') return;
           mraid.util.fireEvent('rotationEvent', azimuth, pitch, roll);
    };

    mraid.util.tiltDetectionEvent = function(x, y) {
           if (state === 'loading') return;
           mraid.util.fireEvent('tiltEvent', x, y);
    };
   ...
   mraid.vibrate = function(duration) {
        if (typeof(duration) != 'number') return;
        mraid.util.nativeCall("mraid://vibrate/?duration="+duration);
   }

   mraid.enableShakeDetection = function(shakeThreshold, debounceTime) {
           if (typeof(shakeThreshold) != 'number') return;
           if (typeof(debounceTime) != 'number') return;

           mraid.util.nativeCall("mraid://enableShakeDetection/?shakeThreshold="+shakeThreshold+"&debounceTime="+debounceTime);
    }

    mraid.enableLongPressDetection = function() {
      mraid.util.nativeCall("mraid://enableLongPressDetection/");
    }

    mraid.getCurrentLocation = function() {
        mraid.util.nativeCall("mraid://getCurrentLocation/");
    }

    mraid.openCamera = function() {
        mraid.util.nativeCall("mraid://openCamera/");
    }

    mraid.enableTiltDetection = function() {
        mraid.util.nativeCall("mraid://enableTiltDetection/");
    }

    mraid.enableRotationDetection = function() {
        mraid.util.nativeCall("mraid://enableRotationDetection/");
    }
